...
def run_calculation():
    ...
    for alpha, radius, l1_ratio in product(alphas, radii,l1_ratios):
        ...
        partial_spot_features = partial(calculate_spot_features, all_input=(
                    bm_data, meta_data, NN_OUTPUT), all_params = (alpha, radius, l1_ratio,[]))
        #feature_results = partial_spot_features(ingested_spot_id[0])
        pool = multiprocessing.Pool(processes=multiprocessing.cpu_count()-1)
        os.system("taskset -p 0xff %d" % os.getpid())
        feature_results = pool.map(partial_spot_features, zip_input, 500)
        pool.close()
        pool.join()
        ...
        for seed in [1,50,200]:
            partial_spot_features = partial(calculate_spot_features, all_input=(
                        bm_data, meta_data, NN_OUTPUT), 
                                            all_params = (alpha, radius, l1_ratio, seed))
            #feature_results = partial_spot_features(ingested_spot_id[0])
            pool = multiprocessing.Pool(processes=multiprocessing.cpu_count()-1)
            os.system("taskset -p 0xff %d" % os.getpid())
            feature_results = pool.map(partial_spot_features,zip_input, 500)
