...
flags.DEFINE_string('train_data_dir', '/home/lun5/Documents/multiplex/MIL/patches/128/', 'Directory containing training data')
flags.DEFINE_string('model_dir', '/home/lun5/Documents/multiplex/MIL/patches/128/model_alex', 'Directory containing training data')
flags.DEFINE_string('train_output_dir', '/home/lun5/Documents/multiplex/MIL/patches/128/data_alex', 'Directory containing training data')
# Feel free to add additional flags to assist in setting hyper parameters
flags.DEFINE_float('L2reg',0.00005 , 'L2 regularization for weights')
flags.DEFINE_float('Adam_lr',1e-7, 'Adam optimizer learning rate')
...
def read_training_list():
    ...
    """
    image_list_file = FLAGS.train_data_dir + 'train_patch_stages.csv'
    f = pd.read_csv(image_list_file)
    filenames = list(f['PatchNames'])
    ...
    bm_filenames = [[FLAGS.train_data_dir + 'biomarker_images' + '/' + bm_name 
                     +'/' + imname for bm_name in biomarkers_names] for imname in filenames]
...
class Fetcher:
    ...
    def load_batch_balanced(self,batchsize):
        num_images_per_class = int(np.floor(batchsize/3))
        sample_indices = []
...
def alexnet(inputs):
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected],
        ...
        net = slim.flatten(net)
        net = slim.fully_connected(net,2048, scope = 'fc')
        net = slim.fully_connected(net, n_classes, activation_fn = None, scope = 'output')
    return net
