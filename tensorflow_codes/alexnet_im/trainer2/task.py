...
#flags.DEFINE_string('train_output_dir', 'gs://broad-cell-lines-ml/simple_augment_batch65/data', 'Directory to put the training data.')
#flags.DEFINE_string('model_dir', 'gs://broad-cell-lines-ml/simple_augment_bach65/model', 'Directory to put the model into.')
flags.DEFINE_string('train_data_dir', '/mnt/lun5/ColonCancerStudy', 'Directory containing training data')
flags.DEFINE_string('model_dir', '/home/lun5/Documents/multiplex/MIL/fullimages/alexnet/model', 'Directory containing training data')
flags.DEFINE_string('train_output_dir', '/home/lun5/Documents/multiplex/MIL/fullimages/alexnet/data', 'Directory containing training data')
# Feel free to add additional flags to assist in setting hyper parameters
flags.DEFINE_float('L2reg',0.005 , 'L2 regularization for weights')
flags.DEFINE_float('Adam_lr',2e-5, 'Adam optimizer learning rate')
flags.DEFINE_float('Adam_beta1',.9 , 'Adam optimizer beta1')
flags.DEFINE_float('Adam_beta2',.999 , 'Adam optimizer beta2')
flags.DEFINE_float('Adam_eps',1e-8 , 'Adam optimizer epsilon')


biomarkers_names = ['BetaCatenin','E_cad','pck26']
im_size = 512
num_channels = len(biomarkers_names)
n_classes = 3
...
def read_training_list():
    ...
    """
    image_list_file = FLAGS.train_data_dir + '/' + 'train_spots_stages.csv'
    f = pd.read_csv(image_list_file)
    filenames = list(f['ImageName'])
    labels = np.asarray(f['Stages'])
    n_classes = len(np.unique(labels))
    onehot = np.vstack([np.asarray(labels == i).astype(int) for i in xrange(n_classes)]).T
    onehot = [onehot[i] for i in xrange(onehot.shape[0])]
    bm_filenames = [[FLAGS.train_data_dir + '/' + slidename + '/AFRemoved/' + bm_name 
                     + '_AFRemoved_'+ str('%03d' %(int(spot_pos))) +'.tif' 
                     for bm_name in biomarkers_names]
                    for slidename, spot_pos in zip(list(f['SlideName']),list(f['pos']))]
    return zip( onehot,bm_filenames),n_classes

def get_mean_im():
    examples, _ = read_training_list()
    ...
    mean_im = np.zeros((im_size, im_size, 3))
    for label, files in val_examples:
        channels = [ np.asarray(Image.open(f).convert('F')) for f in files]
        channels = [misc.imresize(c,(im_size,im_size)) for c in channels]
        mean_im = mean_im + np.dstack(channels)
        
    mean_im /= float(len(val_examples))
    return mean_im
...
class Fetcher:
    ...
    def __init__(self, training_examples):
        ...
        # keep track of which examples were chosen, -1: chosen
    ...
    def load_batch_balanced(self,batchsize):
        ...
        for i in range(n_classes):
            ...
            if num_images < (num_images_per_class*n_classes) + 2: # reset
                labels = [a for a,b,c in self.examples]
                self.current_coded_labels = np.argmax(np.vstack(labels), axis = 1)
            sample_indices.append(np.random.choice(np.nonzero(
                            self.current_coded_labels == i)[0],
                            size = num_images_per_class, replace = False))
        ...
        for i in sample_indices:
            ...
            # You need Pillow
            #channels = [ np.asarray(misc.imread(file_io.FileIO(f,'r'))) for f in files]
            channels = [ np.asarray(Image.open(f).convert('F')) for f in files]
            #if not (channels[0].shape == (im_size, im_size)):
            channels = [misc.imresize(c,(im_size,im_size)) for c in channels]
            # augmentation
            if aug_code == 0:
                channels = channels # do nothing
            elif aug_code == 1:
                channels = [misc.imrotate(channels[j],90) for j in xrange(len(channels))]
            elif aug_code == 2:
                channels = [misc.imrotate(channels[j],180) for j in xrange(len(channels))]
            elif aug_code == 3:
                channels = [misc.imrotate(channels[j],270) for j in xrange(len(channels))]
            elif aug_code == 4:
                channels = [np.fliplr(channels[j]) for j in xrange(len(channels)) ]
            elif aug_code == 5:
                channels = [np.flipud(channels[j]) for j in xrange(len(channels)) ]
            else:
                print('Wrong augmentation type')
            
            x_batch.append((np.dstack(channels) - mean_im)/2e16)
            y_batch.append(label)
            #print(x_batch[-1].shape, y_batch[-1].shape)
            #print('Call np.array')
...
def network(inputs):
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected],
        ...
        net = tf.reshape(inputs,[-1, im_size,im_size,num_channels])
