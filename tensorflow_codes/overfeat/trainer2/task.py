...
flags.DEFINE_string('train_data_dir', '/home/lun5/Documents/mscbio2065/train', 'Directory containing training data')
flags.DEFINE_string('model_dir', '/home/lun5/Documents/mscbio2065/model_alex_128', 'Directory containing training data')
flags.DEFINE_string('train_output_dir', '/home/lun5/Documents/mscbio2065/data_alex_128', 'Directory containing training data')
#flags.DEFINE_string('model_dir', '/home/lun5/Documents/mscbio2065/model_overfeat', 'Directory containing training data')
#flags.DEFINE_string('train_output_dir', '/home/lun5/Documents/mscbio2065/data_overfeat', 'Directory containing training data')
...
class Fetcher:
    ...
    def __init__(self, training_examples):
        ...
        for label,files in training_examples:
            if np.argmax(label) == 3 or np.argmax(label) == 10:
                training_examples_aug.append((label,files,0))
            else:
                for i in range(6):
    ...
    def load_batch_balanced(self,batchsize):
        num_images_per_class = int(np.floor(batchsize/13))
        n_classes = len(labelmap)
        
        sample_indices = []
        for i in range(n_classes):
            num_images = np.sum(self.current_coded_labels == i)
            if num_images < 7: # reset
                labels = [a for a,b,c in self.examples]
                self.current_coded_labels = np.argmax(np.vstack(labels), axis = 1)

            sample_indices.append(np.random.choice(np.nonzero(self.current_coded_labels == i)[0],
...
def run_training():
    ...
    with tf.Graph().as_default():
        ...
        for step in xrange(FLAGS.max_steps):
            ...
            if ((step%500 == 0) and (step>1)) or (step == FLAGS.max_steps-1): # and (step > 1):
